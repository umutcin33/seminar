...
**Erste Ergebnisse:** Bei identischem Input und `temperature=0` schwanken die Ausgaben stark (Java-Fall: 8/10 Läufe mit unterschiedlichen CWE-Mengen, derselbe Bug mit wechselnden, teils falschen CWE-Codes); alle eingebauten Bugs werden auf den kleinen Dateien zwar zuverlässig (10/10) erkannt, die in der Survey genannten Schwächen (Nicht-Determinismus, Halluzination, Prompt-Abhängigkeit) treten also bereits messbar auf.
...
wiederholt denselben Term (z. B. `31608 + 256 = 124·256`) Hunderte Male, bis
...
fand jedoch nur **1 von 4** eingebauten Sicherheitsbugs, naemlich das Memory
Leak (seine klassische Domaene). Buffer Overflow, Signedness und
Integer-Overflow wurden uebersehen (False Negatives); stattdessen meldete
cppcheck Stil- und Info-Hinweise (`unusedFunction`, `unreadVariable`,
`staticFunction`, `missingIncludeSystem`), die keine Sicherheitsluecken sind.
...
und praezise, aber mit enger Abdeckung; das LLM ist breit, aber unzuverlaessig.
Die Werkzeuge sind komplementaer, kein Ersatz fuereinander.
...
klassischen und formalen Methoden. Erste Befunde stützen das: hohe Trefferquote auf kleinem Code, aber deutliche Nicht-Determinismus- und Halluzinationssignale (wechselnde, teils falsche CWE-Codes).
